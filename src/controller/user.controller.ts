...
export class UserController extends BaseController<User> {
  ...
  @Post('/create', { summary: '创建', description: '创建' })
  async create(@Body() user: User): Promise<User> {
    console.log(user);
    console.log(ErrorCode.UN_ERROR);

    Assert.isTrue(!user.username, ErrorCode.UN_ERROR, 'username不能为空');
    Assert.isTrue(!user.password, ErrorCode.UN_ERROR, 'password不能为空');
    Assert.isTrue(!user.phoneNum, ErrorCode.UN_ERROR, 'phoneNum不能为空');
